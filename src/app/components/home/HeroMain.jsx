...
// Find the max width role for min-h calculation
...
const HeroMain = () => {
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden bg-accent-lime" id="home">
      {/* Floating Visual Elements (decorative) */}
      ...
      <section
        ref={ref}
        className="relative h-screen flex items-center w-full z-20"
        tabIndex={-1}
      >
        <div className="w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 h-full">
          <div className="grid grid-cols-1 lg:grid-cols-12 h-full">
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <div>
                    <span className="text-2xl font-black uppercase text-black tracking-wider">Ratan Mia</span>
                    <p className="text-sm text-black/70 font-semibold tracking-wide">IBM Certified Developer</p>
                  </div>
                </motion.div>

                {/* Hero Heading - Fixed-height for stable typewriter */}
                <motion.div
                  className="space-y-2"
                  variants={{
                ...
                >
                  {/* Typewriter role (fixed min-h for no layout shift) */}
                  <div
                    className="w-full"
                  >
                    <h1
                      ...
                      style={{
                        fontSize: 'clamp(2.75rem, 7vw, 5.5rem)', // 44px-88px
                        minHeight: '1.1em', // Reserve at least one line of height
                        maxWidth: '100%',
                        lineHeight: 1.14,
                        whiteSpace: 'nowrap',
                        letterSpacing: '-0.03em',
                        // next line prevents flicker on role change
                    ...
                    >
                      <span
                        aria-live="polite"
                        aria-atomic="true"
                        className="inline-block align-bottom min-w-[14ch]" // [14ch] is safe for your longest role
                        style={{
                          minWidth: `${Math.max(widestRole.length, 11)}ch`,
                          // force size, so typewriter never shrinks
                          display: 'inline-block',
                        }}
                      >
                        {typedRole}
                ...
                >
                  <div>
                    <div className="text-4xl lg:text-5xl font-extrabold text-black mb-1">
                      8+
                    </div>
                ...
                {/* CTAs - desktop stack row, mobile stack col */}
                <motion.div
            ...
            {/* Right Col: Hero Illustration/Image */}
            <div className="col-span-1 lg:col-span-5 flex items-center justify-end h-full relative">
              {/* Maintain image on right for all lg+ and responsive for mobile below */}
              <div className="hidden lg:block absolute right-0 top-0 bottom-0 w-full h-full z-0">
                ...
                >
                  <img
                    ...
                    className="w-full h-full object-cover object-center rounded-bl-[72px] rounded-tl-[32px]"
                    loading="lazy"
                    style={{ minHeight: '100%', maxHeight: '100%', minWidth: '100%' }}
                  />
                  {/* Overlay for contrast */}
                  <div className="absolute inset-0 bg-gradient-to-l from-white/0 via-accent-lime/12 to-accent-lime/60" />
                  <div className="absolute inset-0 bg-gradient-to-t from-accent-lime/20 via-transparent to-transparent" />
                </motion.div>
              </div>
              {/* Mobile image */}
              <div className="lg:hidden w-full h-64 mt-6 rounded-2xl overflow-hidden">
                <img
