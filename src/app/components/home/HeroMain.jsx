...
const CompactHero = () => {
  ...
  return (
    ...
    >
      ...
      <div className="max-w-6xl mx-auto w-full relative z-10 py-8">
        {/* Main Content Grid */}
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <motion.div
            variants={scaleIn}
            className="inline-flex items-center gap-2 bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-400/40 px-4 py-2 rounded-full backdrop-blur-md"
          >
            <motion.div 
            ...
            />
            <span className="text-green-200 font-medium text-xs tracking-wider uppercase">Available For Projects</span>
          </motion.div>

          {/* Main Heading */}
          <motion.div variants={fadeInUp} className="space-y-3">
            <motion.p 
              className="text-blue-200 text-sm md:text-base font-medium tracking-wide uppercase"
              variants={fadeInUp}
            >
            ...
            >
              <motion.span
                key={currentRole}
                className="text-lg md:text-xl font-semibold bg-gradient-to-r from-cyan-400 to-blue-400 bg-clip-text text-transparent"
          ...
          >
            {keyFeatures.map((feature, index) => (
              <motion.div
                key={feature.title}
                className="flex items-center gap-2 p-3 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm"
                whileHover={{ scale: 1.02, y: -1 }}
                transition={{ delay: index * 0.1 }}
              >
                <feature.icon className={`w-4 h-4 ${feature.color} flex-shrink-0`} />
                <div>
                  <div className={`font-medium ${feature.color} text-xs md:text-sm`}>{feature.title}</div>
          ...
          >
            {statistics.map((stat, index) => (
              <motion.div 
                key={stat.label}
                className="text-center p-3 bg-white/5 border border-white/10 rounded-lg backdrop-blur-sm hover:bg-white/10 transition-all duration-300"
              ...
              >
                ...
                <div className={`text-lg md:text-xl font-bold ${stat.textColor}`}>
                  {stat.number}
                </div>
          ...
          <motion.div 
            variants={fadeInUp}
            className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start max-w-lg lg:max-w-none"
          >
            <motion.a
              href="#projects"
              className="group inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-600 to-purple-600 text-white font-semibold rounded-full shadow-lg hover:shadow-xl transition-all duration-300"
            ...
            <motion.a
              href="#contact"
              className="group inline-flex items-center justify-center gap-2 px-6 py-3 bg-transparent border border-white/40 text-white font-semibold rounded-full hover:bg-white hover:text-slate-900 transition-all duration-300"
              whileHover={{ scale: 1.05, y: -1 }}
              whileTap={{ scale: 0.95 }}
            >
            ...
            <motion.a
              href="/resume.pdf"
              download
              className="group inline-flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-emerald-600 to-green-600 text-white font-medium rounded-full transition-all duration-300"
              whileHover={{ scale: 1.05, y: -1 }}
              whileTap={{ scale: 0.95 }}
            >
          ...
          >
            <div className="mb-4 text-center lg:text-left">
              <h4 className="text-lg font-semibold text-white mb-1">Ready to Start?</h4>
              <p className="text-blue-200 text-sm font-light">Let's discuss your project requirements</p>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
              {contactMethods.map((method, index) => (
                <motion.a
                  ...
                  className={`group flex items-center gap-2 p-3 bg-gradient-to-r ${method.color} text-white font-medium rounded-lg transition-all duration-300 hover:shadow-lg`}
        ...
        >
          <div className="relative">
            ...
            >
              ...
              <div className="w-full h-full rounded-full bg-gradient-to-br from-white/10 to-white/5 backdrop-blur-lg border-2 border-white/20 shadow-2xl overflow-hidden relative">
                <div className="w-full h-full bg-gradient-to-br from-blue-500/10 via-purple-500/10 to-pink-500/10 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <div className="text-white font-bold text-xl md:text-2xl mb-2">Ratan Mia</div>
                    <div className="text-blue-200 text-base md:text-lg font-medium mb-2">Full Stack Developer</div>
              ...
              {[
                { icon: Globe, metric: '25+', title: 'Countries', position: '-top-4 -left-4' },
                { icon: Clock, metric: '<2h', title: 'Response', position: '-top-4 -right-4' },
                { icon: Star, metric: '98.5%', title: 'Success', position: '-bottom-4 -right-4' }
              ].map((achievement, index) => (
                <motion.div
                  key={achievement.title}
                  className={`absolute ${achievement.position} bg-white/10 backdrop-blur-lg p-3 rounded-lg border border-white/20 shadow-xl`}
                  initial={{ opacity: 0, scale: 0.8 }}
                  animate={{ opacity: 1, scale: 1 }}
                ...
                >
                  <div className="text-center">
                    <achievement.icon className="w-5 h-5 text-blue-400 mx-auto mb-1" />
                    <div className="text-white font-bold text-sm">{achievement.metric}</div>
            ...
            <div className="absolute inset-0 pointer-events-none">
              {['⚛️', '🚀', '🔧', '💎', '⚡', '🎯'].map((icon, index) => (
                <motion.div
                  key={index}
                  className="absolute w-10 h-10 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center text-lg border border-white/20"
                  style={{
                    top: '50%',
                    left: '50%',
                    transformOrigin: '0 0',
        ...
        >
          ...
          >
            ...
            >
              ...
              <motion.div 
                className="absolute inset-0 w-2 h-2 bg-gradient-to-r from-blue-400 to-purple-400 rounded-full"
                animate={{ 
                  scale: [1, 2, 1],
                  opacity: [0.8, 0, 0.8]
                }}
            ...
            <span className="text-sm font-medium tracking-wide">Discover My Work</span>
