...
import { Calendar, ChevronDown, Code2, ExternalLink, Eye, Globe, Plus, TrendingUp } from 'lucide-react';
...
const Projects = () => {
  ...
  const generatePlaceholderUrl = (title) => {
    ...
    return `https://placehold.co/800x600/e2e8f0/94a3b8?text=${encodedTitle}`;
  };
  ...
  const initialProjects = [
    ...
    },
    {
      ...
      mockupImage: "/images/projects/japan-parts-mockup.png", // Local image path
      status: "Development",
    ...
    },
    {
      ...
      link: null,
      mockupImage: "https://images.unsplash.com/photo-1587560699334-cc4ff634909a?w=800&h=600&fit=crop", // External URL
      status: "Development",
    ...
    },
    {
      ...
      category: "custom-development",
      link: null,
      mockupImage: "https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=800&h=600&fit=crop", // External URL
      status: "Development",
    ...
    },
    {
      ...
      category: "custom-development",
      link: null,
      mockupImage: "https://images.unsplash.com/photo-1563013544-824ae1b704d3?w=800&h=600&fit=crop", // External URL
      status: "Development",
    ...
    },
    {
      ...
      category: "custom-development",
      link: null,
      mockupImage: "https://images.unsplash.com/photo-1576091160399-112ba8d25d1f?w=800&h=600&fit=crop", // External URL
      status: "Live",
  ...
  const allProjects = initialProjects.map(project => {
    if (project.mockupImage === null || project.mockupImage === "") {
      return {
        ...project,
        mockupImage: generatePlaceholderUrl(project.title)
      };
    }
    return project;
  });
              ...
              >
                ...
                >
                  ...
                  <img
                    ...
                    loading="lazy" // Standard HTML5 lazy loading
                    onError={(e) => { 
                      // Fallback if image fails to load (e.g. local path error or broken URL)
                      e.target.onerror = null; // Prevent infinite loop if placeholder also fails
                      e.target.src = generatePlaceholderUrl('Image Load Error'); 
                    }}
                  />
                  ...
                  <AnimatePresence>
                    {hoveredProject === project.id && (
                      <motion.div
                        className="absolute inset-0 bg-black/70 backdrop-blur-sm flex flex-col items-center justify-center text-white z-10 p-4"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                      ...
                      >
                        <div className="text-center">
                          {project.link ? (
                            <>
                              <Globe className="w-10 h-10 mx-auto mb-3 text-green-400" />
                              <h4 className="text-lg font-bold mb-2">{project.title}</h4>
                              <p className="text-xs bg-white/20 px-3 py-1.5 rounded-full mb-4 font-mono break-all max-w-full truncate">
                                {project.link.replace('https://', '').replace('http://', '')}
                              </p>
                              <motion.button
                                type="button"
                                className="inline-flex items-center gap-2 px-5 py-2.5 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg shadow-lg text-sm"
                                whileHover={{ scale: 1.05 }}
                                whileTap={{ scale: 0.95 }}
                            ...
                            <>
                              <Code2 className="w-10 h-10 mx-auto mb-3 text-blue-400" />
                              <h4 className="text-lg font-bold mb-2">{project.title}</h4>
                              <p className="text-sm text-gray-300 mb-4">
                                Tailored solution for {project.client}
                              </p>
                              <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-blue-500 text-white font-semibold rounded-lg text-sm">
